...
interface UserProgressDao {
    ...
    suspend fun getProgressByItemId(itemId: Int, itemType: String): UserProgressEntity?
    ...
    fun getProgressByItemIdFlow(itemId: Int, itemType: String): Flow<UserProgressEntity?>
    ...
    suspend fun getProgressByItemIds(itemIds: List<Int>, itemType: String): List<UserProgressEntity>

    @Query("UPDATE user_progress SET is_favorite = :isFavorite, updated_at = :updatedAt WHERE item_type = :itemType AND item_id = :itemId")
    suspend fun updateFavoriteStatus(itemId: Int, itemType: String, isFavorite: Boolean, updatedAt: String)

    @Query("UPDATE user_progress SET state = :state, updated_at = :updatedAt WHERE item_type = :itemType AND item_id = :itemId")
    suspend fun updateProgressState(itemId: Int, itemType: String, state: Int, updatedAt: String)
    ...
    suspend fun getByItemIdSync(itemId: Int, itemType: String): UserProgressEntity?

    @Query("DELETE FROM user_progress WHERE item_id IN (:itemIds) AND item_type = :itemType")
    suspend fun deleteByItemIds(itemIds: List<Int>, itemType: String)
}
