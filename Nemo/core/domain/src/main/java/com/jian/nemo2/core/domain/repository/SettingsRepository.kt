...
interface SettingsRepository {
     ...
     */
    suspend fun setThemeColor(colorArgb: Long?)
    ...
    /**
     * 深色模式策略Flow
     * "system" = 跟随系统, "scheduled" = 定时切换
     */
    val darkModeStrategyFlow: Flow<String>

    /**
     * 设置深色模式策略
     */
    suspend fun setDarkModeStrategy(strategy: String)

    /**
     * 浅色模式开始时间Flow (HH:mm)
     */
    val darkModeStartTimeFlow: Flow<String>
     ...
     * 深色模式开始时间Flow (HH:mm)
     */
    val darkModeEndTimeFlow: Flow<String>
    ...
    suspend fun addTodayTestedWordId(id: Long)
    suspend fun addTodayWrongWordId(id: Long)
    suspend fun addTodayTestedGrammarId(id: Long)
    suspend fun addTodayWrongGrammarId(id: Long)

    // P3修复: 获取今日测试的单词或语法ID
    suspend fun getTodayTestedWordIds(): Set<Long>
    suspend fun getTodayTestedGrammarIds(): Set<Long>
     ...
     * 默认: "N5"
     */
    val preferredWordLevelFlow: Flow<String>

    /**
     * 设置偏好单词等级
     */
    suspend fun setPreferredWordLevel(level: String)

    /**
     * 偏好语法等级Flow
     * 默认: "N5"
     */
    val preferredGrammarLevelFlow: Flow<String>

    /**
     * 设置偏好语法等级
     */
    suspend fun setPreferredGrammarLevel(level: String)
     ...
     */
    suspend fun saveWordSession(ids: List<Long>, currentIndex: Int, level: String, steps: Map<Long, Int>, waitingUntil: Long = 0L)
     ...
     * @param waitingUntil 等待结束时间 (Epoch Millis)
     */
    suspend fun saveGrammarSession(ids: List<Long>, currentIndex: Int, level: String, steps: Map<Long, Int>, waitingUntil: Long = 0L)
    ...
    /** 记忆算法参数 (Lapse) */
    val wordLapsesFlow: Flow<Map<Long, Int>>
    val grammarLapsesFlow: Flow<Map<Long, Int>>

    suspend fun incrementWordLapse(wordId: Long)
    suspend fun incrementGrammarLapse(grammarId: Long)

    suspend fun resetWordLapse(wordId: Long)
    suspend fun resetGrammarLapse(grammarId: Long)
    ...
    /** FSRS 目标留存率Flow 默认: 0.9 */
    val fsrsTargetRetentionFlow: Flow<Double>
    suspend fun setFsrsTargetRetention(retention: Double)
     ...
     */
    suspend fun getAppSettingsSnapshot(): com.jian.nemo2.core.domain.model.AppSettings

    /**
     * Apply app settings from sync
     */
    suspend fun applyAppSettingsSnapshot(settings: com.jian.nemo2.core.domain.model.AppSettings)
...
data class SessionData(
    ...
    val level: String,
    val steps: Map<Long, Int>, // ID -> Step Index
