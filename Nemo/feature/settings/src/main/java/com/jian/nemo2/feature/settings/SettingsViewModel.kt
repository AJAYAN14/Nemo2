...
class SettingsViewModel @Inject constructor(
    ...
    private val resetProgressUseCase: com.jian.nemo2.core.domain.usecase.settings.ResetProgressUseCase,
    private val playTtsUseCase: com.jian.nemo2.core.domain.usecase.audio.PlayTtsUseCase,
    private val audioRepository: com.jian.nemo2.core.domain.repository.AudioRepository,
    private val studyRepository: com.jian.nemo2.core.domain.repository.StudyRepository,
    private val syncManager: com.jian.nemo2.core.domain.service.SyncManager,
    ...
    private fun observeTtsEvents() {
        viewModelScope.launch {
            audioRepository.ttsEvents.collect { event ->
                val id = when (event) {
                    is com.jian.nemo2.core.domain.repository.TtsEvent.OnStart -> event.id
                    is com.jian.nemo2.core.domain.repository.TtsEvent.OnDone,
                    is com.jian.nemo2.core.domain.repository.TtsEvent.OnError,
                    com.jian.nemo2.core.domain.repository.TtsEvent.GoogleTtsMissing -> null
                }

                if (id?.startsWith("preview-") == true) {
                    when (event) {
                        is com.jian.nemo2.core.domain.repository.TtsEvent.OnStart -> {
                            // 开始播放时，状态已由 onEvent 提前设置
                        }
                        is com.jian.nemo2.core.domain.repository.TtsEvent.OnDone,
                        is com.jian.nemo2.core.domain.repository.TtsEvent.OnError,
                        com.jian.nemo2.core.domain.repository.TtsEvent.GoogleTtsMissing -> {
                            // 结束或错误时清除预览状态
    ...
    private fun loadSettings() {
        viewModelScope.launch {
            ...
            val advancedFlow = combine(
                 ...
                 settingsRepository.fsrsTargetRetentionFlow
            ) { args ->
                AdvancedSettings(
                    ...
                    fsrsTargetRetention = args[5] as Double
                )
            }
            ...
            combine(
                ...
                _uiState.update { state ->
                    state.copy(
                        ...
                        fsrsTargetRetention = advanced.fsrsTargetRetention,
                        ttsSpeechRate = rate,
                        ttsPitch = pitch,
    ...
    fun onEvent(event: SettingsEvent) {
        when (event) {
            ...
            is SettingsEvent.ClearStatusMessage -> _uiState.update { it.copy(statusMessage = null) }
            is SettingsEvent.RestoreDefaultAdvancedLearningSettings -> restoreDefaultAdvancedLearningSettings()
    ...
    private fun restoreDefaultAdvancedLearningSettings() {
        ...
        updateStatusMessage("已恢复默认配置", 2000)
    }
    ...
    private fun resetProgress(includeCloud: Boolean) {
        viewModelScope.launch {
            ...
            when (val result = resetProgressUseCase(includeCloud)) {
                is com.jian.nemo2.core.common.Result.Success -> {
                    _uiState.update { it.copy(isLoading = false) }
                    updateStatusMessage("学习进度已重置，所有学习数据已清除", 5000)
                }
                is com.jian.nemo2.core.common.Result.Error -> {
                    _uiState.update { it.copy(isLoading = false) }
...
private data class AdvancedSettings(
    ...
    val fsrsTargetRetention: Double
)
